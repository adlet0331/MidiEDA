...
class MAESTRO(PianoRollAudioDataset):
    ...
    def __init__(self, path='data/maestro/maestro-v3.0.0', groups=None, sequence_length=None, seed=42, device=DEFAULT_DEVICE):
...
class PIANOVAM(PianoRollAudioDataset):

    def __init__(self, path='data/pianovam', groups=None, sequence_length=None, seed=42, device=DEFAULT_DEVICE):
        super().__init__(path, groups if groups is not None else ['train'], sequence_length, seed, device)
    ...
    def files(self, group):
        ...
        files = sorted([
            (
                os.path.join(self.path, "audio", row["record_time"] + ".flac"),
                os.path.join(self.path, "midi", row["record_time"] + ".mid")
            ) 
            for row in metadata if row.get("split") == group
        ])
