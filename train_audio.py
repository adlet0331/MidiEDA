...
def my_config():
    ...
    logdir = 'runs-audio/p-est-' + datetime.now().strftime('%y%m%d-%H%M%S')
...
def train(logdir, numeric_features, numeric_versions, device, iterations, batch_size, 
    ...
    trainset, validset, testset = random_split(
        ...
        [int(len(dataset) * 0.6),
         int(len(dataset) * 0.2),
         len(dataset) - int(len(dataset) * 0.6) - int(len(dataset) * 0.2)]
    )
    max_label = dataset.get_max_label()
    ...
    features_list = []
    features_name_list = dataset.features_names
    ...
    if save_log:
        import wandb
        wandb.init(
            project="RubricNet-AudioTranscription",
            name=logdir,
            config={
                "logdir": logdir,
    ...
    while iteration < iterations:
        for batch_features, batch_labels in train_dataloader:
            ...
            batch_labels = (torch.arange(1, max_label + 1, device=device)[None, :] <= batch_labels[:, None]).to(torch.float32)
            ...
            if iteration % validation_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    for valid_batch in valid_dataloader:
                        ...
                        valid_batch_labels = (
                            torch.arange(1, max_label + 1, device=device)[None, :] <= valid_batch_labels[:, None]
                        ).to(torch.float32)
